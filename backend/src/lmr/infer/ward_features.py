...
def configure(
    ...
    ward_boundaries_key: str,
    output_bucket: str | None = None,
) -> None:
    """
    Override module-level S3 config. Call before main().

    Parameters
    ----------
    source_data_bucket : str
        S3 bucket containing pre-downloaded satellite parquets.
    source_data_prefix : str
        S3 key prefix to the parquet files (e.g. "path/to/planetary_computer").
    ward_boundaries_key : str
        S3 key for the Kenya ADM3 ward boundary GeoJSON within source_data_bucket.
    output_bucket : str, optional
        Bucket for output parquets. Defaults to source_data_bucket.
    """
    global SM_BUCKET, PC_PREFIX, WARD_BOUNDARIES_KEY, s3
    SM_BUCKET = output_bucket or source_data_bucket
    PC_PREFIX = source_data_prefix
    WARD_BOUNDARIES_KEY = ward_boundaries_key
    # Re-initialize S3 client (in case region differs)
    s3 = boto3.client("s3")
...
# CLI entry point removed — use lmr.infer.feature_extract.run_feature_extraction()
# or call main() directly after configure().
